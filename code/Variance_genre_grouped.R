...
# Créer la variable MusicPlacesGroup à partir des lieux d’écoute en italien
group_1 <- c("Locale", "In strada", "Serata in appartamento", "In auto", "In strada")
group_2 <- c( "Chiesa")
group_3 <- c("Altro")

# Créer une colonne de groupe à partir des réponses (si plusieurs colonnes, il faudra adapter)
df$MusicPlacesGroup <- NA

df$MusicPlacesGroup[df$MusicPlaces %in% group_1] <- "G1"
df$MusicPlacesGroup[df$MusicPlaces %in% group_2] <- "G2"
df$MusicPlacesGroup[df$MusicPlaces %in% group_3] <- "G3"

df$MusicPlacesGroup <- as.factor(df$MusicPlacesGroup)
...
other_predictors <- c("City", "Incomes", "MusicPlacesGroup")
